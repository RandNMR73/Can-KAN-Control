...
from gym_cartlataccel.new_env import BatchedCartLatAccelEnv as CartLatAccelEnv
...
class PPO:
  def __init__(self, env, model1, model2, lr=1e-1, gamma=0.99, lam=0.95, clip_range=0.2, epochs=1, n_steps=200, ent_coeff=0.01, bs=100, env_bs=1, device='cuda', debug=False, seed=42):
    self.env = env
    self.env_bs = env_bs
  ...
  @staticmethod
  def rollout(self, max_steps=1000, deterministic=False):
    states, actions, rewards, dones  = [], [], [], []
    state, _ = self.env.reset()
    for _ in range(max_steps):
      state_tensor = torch.FloatTensor(state).to(self.device)
      action1 = self.models[0].get_action(state_tensor, deterministic=deterministic)
      action2 = self.models[1].get_action(state_tensor, deterministic=deterministic)
      
      next_state1, reward1, terminated1, truncated1, info1 = self.env.step(action1)
      next_state2, reward2, terminated2, truncated2, info2 = self.env.step(action2)
      
      states.append(state)
      actions.append((action1, action2))
      rewards.append((reward1, reward2))
      dones.append((terminated1 or truncated1, terminated2 or truncated2))
      state = (next_state1, next_state2)
  ...
  def train(self, max_evals=1000):
    ...
    while eps < max_evals:
      ...
      states, actions, rewards, dones, next_state = self.rollout(self.n_steps)
      rollout_time = time.perf_counter() - start
      
      for i in range(2):
        model = self.models[i]
        action_set = [a[i] for a in actions]
        reward_set = [r[i] for r in rewards]
        done_set = [d[i] for d in dones]
        
        start = time.perf_counter()
        with torch.no_grad():
          state_tensor = torch.FloatTensor(np.array(states)).to(self.device)
          next_state_tensor = torch.FloatTensor(next_state[i]).to(self.device)
          action_tensor = torch.FloatTensor(np.array(action_set)).to(self.device)
        ...
        returns, advantages = self.compute_gae(np.array(reward_set), values, np.array(done_set), next_values)
        ...
        for _ in range(self.epochs):
          for batch in self.replay_buffer:
            ...
            loss = self.evaluate_cost(model, batch, advantages)
...
if __name__ == "__main__":
  ...
  if args.model == "kan":
    model1 = KANActorCritic(env.observation_space.shape[-1], {"pi": [args.hidden_sizes], "vf": [32]}, env.action_space.shape[-1], act_bound=(-1,1))
    model2 = KANActorCritic(env.observation_space.shape[-1], {"pi": [args.hidden_sizes], "vf": [32]}, env.action_space.shape[-1], act_bound=(-1,1))
  else:
    model1 = ActorCritic(env.observation_space.shape[-1], {"pi": [args.hidden_sizes], "vf": [32]}, env.action_space.shape[-1], act_bound=(-1,1))
    model2 = ActorCritic(env.observation_space.shape[-1], {"pi": [args.hidden_sizes], "vf": [32]}, env.action_space.shape[-1], act_bound=(-1,1))
  device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
  ppo = PPO(env, model1, model2, env_bs=args.env_bs, device=device, seed=args.seed, debug=True)
