...
class BatchedCartLatAccelEnv(gym.Env):
  ...
  def step(self, action1, action2):
    theta_prev = np.transpose(self.env.state[:, :-1])
    target = self.env.state[:, -1]

    actions = [action1, action2]
    new_states, rewards, truncated, info = [], [], False, []
    for i, action in enumerate(actions):
      scaled_action = action * (np.array(self.env.high) - np.array(self.env.low)) + np.array(self.env.low)
      theta = scaled_action
      x = self.env.fx(torch.tensor(theta)).detach().cpu().numpy()
      x, theta = np.transpose(x), np.transpose(theta)
      new_target = self.env.x_targets[:, self.env.curr_step]
      noisy_target = self.env.noise_model.add_lat_noise(self.env.curr_step, new_target)
      
      self.env.state = np.stack(np.concatenate((theta, new_target.reshape(1, -1)), axis=0), axis=1)
      self.env.obs = np.stack(np.concatenate((theta, noisy_target.reshape(1, -1)), axis=0), axis=1)

      alpha = 0.1
      step_weight = 1 - (self.env.curr_step / self.env.max_episode_steps)
      dist = abs(x - target)
      jerk = abs(theta - theta_prev)
      error = np.sum(dist + alpha * jerk, axis=0)
      reward = -error * step_weight / (self.env.max_x - self.env.min_x)

      new_states.append(np.array(self.env.obs, dtype=np.float32))
      rewards.append(reward)
      info.append({"action": action, "noisy_action": theta, "x": x, "x_target": new_target})
    
    self.env.curr_step += 1
    truncated = self.env.curr_step >= self.env.max_episode_steps
    return new_states, rewards, False, truncated, info
